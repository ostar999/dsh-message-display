 ...
 * （Host 半边的 /dsh-message-navigator/fetch-history 路由），滚动到列表顶部继续加载。
 */
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const HISTORY_ROUTE = '/dsh-message-navigator/fetch-history'
...
`
    ...
    exports.name = 'dsh-message-navigator'
    ...
    exports.apply = function apply(ctx) {
        ...
        const loadOlder = async () => {
          ...
          try {
            ...
            if (result !== null && typeof result === 'object' && result.ok === true && Array.isArray(result.items)) {
              ...
              console.error('[dsh-message-navigator] fetch-history failed:', message)
            }
          } catch (error) {
            const message = String(error && error.message ? error.message : error)
            setOlderStatus({ kind: 'error', message })
            console.error('[dsh-message-navigator] fetch-history threw:', error)
          } finally {
      ...
      ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        { name: 'shell.overlay', id: 'dsh-message-navigator', order: 50, label: '消息导航' },
